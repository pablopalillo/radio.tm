...
function tm_edit_schedule_field($taxonomy) {
	...
	$html .= '			<label for="taxonomy_day[]">' . __('Day', 'tmr') . '</label>'.PHP_EOL;
	$html .= '			<select name="taxonomy_day[]">'.PHP_EOL;
	...
	$html .= '			</select>'.PHP_EOL;
	$html .= '				<label for="taxonomy_hour[]">' . __('Schedule', 'tmr') . '</label>'.PHP_EOL;
	$html .= '				<input type="number" max="23" min="0" name="taxonomy_hour[]" class="taxonomy_hour" value="'.$days[$i]['hour'].'"  />'.PHP_EOL;
	$html .= '				<input type="number" max="59" min="0" step="5" name="taxonomy_minute[]" class="taxonomy_minute" value="'.$days[$i]['minute'].'" />'.PHP_EOL;
	$html .= '				<label for="taxonomy_duration[]">' . __('Duration', 'tmr') . '</label>'.PHP_EOL;
	$html .= '				<input type="number" min="1" name="taxonomy_duration[]" class="taxonomy_duration" value="'.$days[$i]['duration'].'" />'.PHP_EOL;
...
function tm_script() {
	return '<script type="text/javascript">
		function add_day(event)
		{
			...
			dia += "	<label for=\"taxonomy_day[]\">' . __('Day', 'tmr') . '</label>";
			dia += "	<select name=\"taxonomy_day[]\">";
			...
			dia += "	</select>";
			dia += "		<label for=\"taxonomy_hour[]\">' . __('Schedule', 'tmr') . '</label>";
			dia += "		<input type=\"number\" max=\"23\" min=\"0\" name=\"taxonomy_hour[]\" class=\"taxonomy_hour\" value=\"14\">";
			dia += "		<input type=\"number\" max=\"59\" min=\"0\" step=\"5\" name=\"taxonomy_minute[]\" class=\"taxonomy_minute\" value=\"0\">";
			dia += "		<label for=\"taxonomy_duration[]\">' . __('Duration', 'tmr') . '</label>";
			dia += "		<input type=\"number\" min=\"1\" name=\"taxonomy_duration[]\" class=\"taxonomy_duration\" value=\"60\">";
...
function tm_save_taxonomy_schedule($term_id) {
	...
	if(isset($_POST['count']) && $_POST['count'] > 0)
    {
    	...
    	for($i = 0; $i < $count; $i++)
    	{
    		$dia 		= $_POST['taxonomy_day'][$i];
    		$hora 		= $_POST['taxonomy_hour'][$i];
    		$minuto 	= $_POST['taxonomy_minute'][$i];
    		$duracion 	= $_POST['taxonomy_duration'][$i];
